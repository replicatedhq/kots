...
func Start() {
	...
	r.Path("/api/v1/app/{appSlug}/versions").Methods("OPTIONS", "GET").HandlerFunc(handlers.GetAppVersionHistory)
	...
	r.Path("/api/v1/kurl/nodes").Methods("OPTIONS", "GET").HandlerFunc(handlers.GetKurlNodes)
	...
	r.Path("/api/v1/gitops/create").Methods("OPTIONS", "POST").HandlerFunc(handlers.CreateGitOps)
