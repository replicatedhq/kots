...
async function getPreflightResult({
  ...
  if (!jsonResponse.ok) {
    throw new Error(`Encountered an error while fetching preflight results: Unexpected status code: ${jsonResponse.status}`);
  }
  ...
  if (typeof response?.preflightResult?.result === "string")
    response.preflightResult.result =
      JSON.parse(response.preflightResult.result);
...
function flattenPreflightResponse({ refetchCount, response }:
  { refetchCount: number, response: PreflightResponse, }): PreflightCheck {

  if (typeof response?.preflightProgress === "string" || typeof response?.preflightResult?.result === "string")
    throw new Error("Preflight response is not properly unmarshalled");

  return {
    // flatten the error strings out into an array
    errors: response?.preflightResult?.result
      ?.errors?.map(error => error.error) || [],
    pendingPreflightCheckName: response?.preflightProgress?.currentName || "",
    // TODO: see if we can calculate a real %
    pendingPreflightChecksPercentage: refetchCount === 0 ? 0 :
      refetchCount > 21 ? 96 : (refetchCount * 4.5),
    preflightResults: response?.preflightResult?.result?.results
      ?.map(responseResult => ({
    ...
    showDeploymentBlocked: response?.preflightResult?.result?.results
      ?.find(result => result?.isFail && result?.strict) ? true : false,
    showPreflightCheckPending: !response?.preflightResult?.result,
    showPreflightNoChecks: response?.preflightResult?.result
      ?.results?.length === 0,
    showPreflightSkipped: response?.preflightResult?.skipped,
    showRbacError: response?.preflightResult?.result
      ?.errors?.find(error => error?.isRbac) ? true : false,
  }
}
...
function useGetPrelightResults({ slug, sequence }:
  { slug: string; sequence?: number; }) {
