...
class SnapshotRestore extends Component {
  ...
  renderWarnings = (warnings) => {
    return warnings?.map((warning, i) => (
      ...
      >
        <Icon
          icon={"warning-circle-filled"}
          className="warning-color flex-auto u-marginRight--10 u-marginLeft--10"
          size={16}
        />
        <p className="u-fontSize--normal u-fontWeight--normal u-lineHeight--normal">
          {warning.message}
    ...
    return (
      <div className="FailedRestore--wrapper">
        <div className="flex flex-column alignItems--center">
          ...
          <p className="u-fontWeight--bold u-textColor--primary u-fontSize--larger u-lineHeight--normal u-marginTop--15 u-marginBottom--10">
            Partial restore was restored with warnings{" "}
          </p>
          <p className="u-fontSize--normal u-fontWeight--medium u-textColor--bodyCopy u-lineHeight--normal">
  ...
  render() {
    ...
    const hasNoErrorsOrWarnings =
      (!currentAppDetails?.restoreDetail?.warning &&
        !currentAppDetails?.restoreDetail?.errors) ||
      (currentAppDetails?.restoreDetail?.warnings?.length === 0 &&
        currentAppDetails?.restoreDetail?.errors?.length === 0);
    const restoreCompleted = isRestoreSucceeded(currentAppDetails?.restoreDetail?.phase);
    const restoreFailing = isRestoreFailed(currentAppDetails?.restoreDetail?.phase);
    const restoreLoading =
      !currentAppDetails?.restoreDetail?.warnings &&
      !currentAppDetails?.restoreDetail?.errors;
