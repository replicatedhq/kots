  ...
  render() {
    ...
    return (
      <div className={classNames("flex1 flex-column u-padding--20 alignItems--center u-overflow--auto")}>
        {fromLicenseFlow && getKotsApp?.getKotsApp && <span className="u-fontSize--larger u-color--tuna u-fontWeight--bold" style={{ marginTop: "auto" }}>Configure {getKotsApp.getKotsApp.name}</span>}
        ...
        <button className="btn secondary green u-marginTop--20" disabled={savingConfig || (!changed && !fromLicenseFlow)} onClick={this.handleSave} style={{ marginBottom: "auto" }}>{savingConfig ? "Saving" : fromLicenseFlow ? "Continue" : "Save config"}</button>
      </div>
    )
  }
}
