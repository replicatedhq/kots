...
import { listWatches } from "../../../queries/AppsQueries";
import * as Pact from "@pact-foundation/pact";
import { Matchers } from "@pact-foundation/pact";
import { listWatchesRaw } from "../../../queries/AppsQueries";
...
export default () => {
  ...
  it("lists watches for ship-cluster account", (done) => {
    ...
    .then(result => {
      expect(result.data.listWatches.watches).to.have.lengthOf(1);

      expect(result.data.listWatches.watches[0].id).to.equal("better-db-midstream");
      expect(result.data.listWatches.watches[0].watchName).to.equal("Better DB Midstream");
      expect(result.data.listWatches.watches[0].slug).to.equal("ship-cluster-account/better-db-midstream");
      expect(result.data.listWatches.watches[0].watches).to.have.length(1);

      const childWatch = result.data.listWatches.watches[0].watches[0];
