...
export class KotsDeploySocketService {
  ...
  async undeployApp(app: KotsApp, cluster: Cluster): Promise<void> {
    ...
    const rendered = await app.render(app.currentSequence!.toString(), `overlays/downstreams/${cluster.title}`, kotsAppSpec?.kubectlVersion);
          ...
          if (clusterSocketHistory.sentDeploySequences.indexOf(`${app.id}/${deployedAppSequence}`) === -1) {
            ...
            try {
              ...
              if (previousSequence !== undefined) {
                const previousRendered = await app.render(previousSequence.toString(), `overlays/downstreams/${cluster.title}`, kotsAppSpec?.kubectlVersion);
