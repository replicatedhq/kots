...
func UpstreamUpgradeCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			upgradeOptions.GetAppEndpoint = fmt.Sprintf("http://localhost:%d/api/v1/app/%s", localPort, url.PathEscape(appSlug))
			upgradeOptions.VersionHistoryEndpoint = fmt.Sprintf("http://localhost:%d/api/v1/app/%s/versions", localPort, url.PathEscape(appSlug))
