...
func StartHelmConfigServer(port int, chartName string, chartPath string, chartVersion string, chartRootDir string) {
	r := mux.NewRouter()

	r.Use(handlers.CorsMiddleware)
	r.Methods("OPTIONS").HandlerFunc(handlers.CORS)

	handler := &handlers.HelmConfigHandler{
		ChartName:         chartName,
		ChartPath:         chartPath,
		ChartVersion:      chartVersion,
		ChartRootDir:      chartRootDir,
		TempConfigValues:  map[string]kotsv1beta1.ConfigValue{},
		ConfigValuesSaved: false,
	}
	r.Path("/api/v1/ping").Methods("GET").HandlerFunc(handler.Ping)
	handlers.RegisterConfigRoutes(r, handler)
	...
	**********************************************************************/
	...
	if os.Getenv("DISABLE_SPA_SERVING") != "1" {
		...
		u, err := url.Parse(os.Getenv("WEB_PROXY_UPSTREAM"))
		if err != nil {
			panic(err)
		}
		upstream := httputil.NewSingleHostReverseProxy(u)
