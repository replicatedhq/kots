...
func Test_getLatestDeployableDownstreamVersion(t *testing.T) {
	tests := []struct {
		name                      string
		versions                  *downstreamtypes.DownstreamVersions
		expectedLatestDeployable  *downstreamtypes.DownstreamVersion
		expectedSkippedVersions   int
		expectedRemainingVersions int
	...
	}{
		{
			...
			expectedLatestDeployable:  nil,
			expectedSkippedVersions:   0,
			expectedRemainingVersions: 0,
			expectedError:             "no versions found for app",
		},
		{
			name: "all pending versions are demoted - should return nil",
			versions: &downstreamtypes.DownstreamVersions{
				AllVersions: []*downstreamtypes.DownstreamVersion{
					{
						Sequence:   3,
		...
		{
			name: "all pending versions are demoted including mixed required/non-required",
			versions: &downstreamtypes.DownstreamVersions{
				AllVersions: []*downstreamtypes.DownstreamVersion{
					{
						Sequence:   4,
		...
		{
			name: "only demoted required versions in pending",
			versions: &downstreamtypes.DownstreamVersions{
				AllVersions: []*downstreamtypes.DownstreamVersion{
					{
						Sequence:   3,
		...
		{
			name: "empty pending versions list",
			versions: &downstreamtypes.DownstreamVersions{
				AllVersions: []*downstreamtypes.DownstreamVersion{
					{
						Sequence:   1,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			// For tests that would return non-nil latestDeployableVersion,
			// we skip the actual execution to avoid mocking AddDownstreamVersionDetails
			if test.expectedLatestDeployable != nil {
				t.Skip("Skipping test that would trigger AddDownstreamVersionDetails")
				return
			}

			latestDeployable, skipped, remaining, err := store.getLatestDeployableDownstreamVersion("test-app", "test-cluster", test.versions)
			...
			assert.Equal(t, test.expectedLatestDeployable, latestDeployable)
			assert.Equal(t, test.expectedSkippedVersions, skipped)
			assert.Equal(t, test.expectedRemainingVersions, remaining)
		})
	}
}
