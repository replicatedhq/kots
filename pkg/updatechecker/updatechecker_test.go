...
func Test_maybeUpdatePendingVersionsMetadata(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "pending version is demoted but found in upstream updates - should undemote",
			appID: "test-app",
			currentVersion: &downstreamtypes.DownstreamVersion{
				UpdateCursor: "cursor-1",
				ChannelID:    "channel-1",
		...
		{
			name:  "pending version is not demoted but not found in upstream updates - should demote",
			appID: "test-app",
			currentVersion: &downstreamtypes.DownstreamVersion{
				UpdateCursor: "cursor-1",
				ChannelID:    "channel-1",
			...
			mockUpdates: []upstreamtypes.Update{
				{
					VersionLabel: "v1.0.0",
					ChannelID:    "channel-1",
					Cursor:       "cursor-3", // Different cursor - version 2 not found
				},
			},
			setupMockStore: func(mockStore *mock_store.MockStore) {
				mockStore.EXPECT().UpdateAppVersionMetadata("test-app", gomock.Any()).Return(nil)
		...
		{
			name:  "pending version already demoted and not in upstream - no action needed",
			appID: "test-app",
			currentVersion: &downstreamtypes.DownstreamVersion{
				UpdateCursor: "cursor-1",
				ChannelID:    "channel-1",
			...
			setupMockStore: func(mockStore *mock_store.MockStore) {
				mockStore.EXPECT().UpdateAppVersionMetadata("test-app", gomock.Any()).Return(nil)
				// No demotion call expected since it's already demoted
			},
		},
	}
