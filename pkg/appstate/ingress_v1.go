...
func runIngressV1Controller(
	...
	eventHandler := NewIngressV1EventHandler(
		clientset,
...
type ingressV1EventHandler struct {
	clientset       kubernetes.Interface
	informers       []types.StatusInformer
	resourceStateCh chan<- types.ResourceState
}

func NewIngressV1EventHandler(clientset kubernetes.Interface, informers []types.StatusInformer, resourceStateCh chan<- types.ResourceState) *ingressV1EventHandler {
	return &ingressV1EventHandler{
		clientset:       clientset,
		informers:       informers,
...
func (h *ingressV1EventHandler) ObjectCreated(obj interface{}) {
	r := h.cast(obj)
	if _, ok := h.getInformer(r); !ok {
		return
	}
	h.resourceStateCh <- makeIngressV1ResourceState(r, calculateIngressV1State(h.clientset, r))
}

func (h *ingressV1EventHandler) ObjectUpdated(obj interface{}) {
	r := h.cast(obj)
	if _, ok := h.getInformer(r); !ok {
		return
	}
...
func (h *ingressV1EventHandler) ObjectDeleted(obj interface{}) {
	r := h.cast(obj)
	if _, ok := h.getInformer(r); !ok {
		return
	}
	h.resourceStateCh <- makeIngressV1ResourceState(r, types.StateMissing)
}

func (h *ingressV1EventHandler) cast(obj interface{}) *networkingv1.Ingress {
	r, _ := obj.(*networkingv1.Ingress)
	return r
}

func (h *ingressV1EventHandler) getInformer(r *networkingv1.Ingress) (types.StatusInformer, bool) {
	if r != nil {
...
func makeIngressV1ResourceState(r *networkingv1.Ingress, state types.State) types.ResourceState {
...
func calculateIngressV1State(clientset kubernetes.Interface, r *networkingv1.Ingress) types.State {
	...
	states = append(states, ingressV1GetStateFromBackend(clientset, ns, *backend))
	for _, rules := range r.Spec.Rules {
		for _, path := range rules.HTTP.Paths {
			states = append(states, ingressV1GetStateFromBackend(clientset, r.Namespace, path.Backend))
		}
	}
	...
	states = append(states, ingressV1GetStateFromExternalIP(r))
	return types.MinState(states...)
}

func ingressV1GetStateFromBackend(clientset kubernetes.Interface, namespace string, backend networkingv1.IngressBackend) (minState types.State) {
...
func ingressV1GetStateFromExternalIP(ing *networkingv1.Ingress) types.State {
