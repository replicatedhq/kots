...
func Test_ReplicatedAppEndpoint(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantPanic {
				req.Panics(func() {
					_ = ReplicatedAppEndpoint(&licensewrapper.LicenseWrapper{V1: tt.license})
				})
				return
			}

			result := ReplicatedAppEndpoint(&licensewrapper.LicenseWrapper{V1: tt.license})
...
func TestIsV3EmbeddedCluster(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "V3 env var set to true",
			envValue: "true",
			expected: true,
		},
		{
			name:     "V3 env var set to false",
			envValue: "false",
			expected: false,
		},
		{
			name:     "V3 env var set to random value",
			envValue: "random",
			expected: false,
		},
		{
			name:     "V3 env var unset",
			envValue: "",
			expected: false,
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if test.envValue != "" {
				t.Setenv("IS_EMBEDDED_CLUSTER_V3", test.envValue)
			}
			result := IsV3EmbeddedCluster()
			assert.Equal(t, test.expected, result)
		})
	}
}

func TestIsV3EmbeddedClusterInitialInstall(t *testing.T) {
	tests := []struct {
		name     string
		envValue string
		sequence int64
		...
		{
			name:     "V3 initial install - sequence 0",
			envValue: "true",
			sequence: 0,
			expected: true,
		},
		{
			name:     "V3 upgrade - sequence 1",
			envValue: "true",
			sequence: 1,
			expected: false,
		},
		{
			name:     "Non-V3 initial install - sequence 0",
			envValue: "false",
			sequence: 0,
			expected: false,
		},
		{
			name:     "Non-V3 upgrade - sequence 1",
			envValue: "false",
			sequence: 1,
			expected: false,
		},
		{
			name:     "V3 unset - sequence 0",
			envValue: "",
			sequence: 0,
			expected: false,
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			result := IsV3EmbeddedClusterInitialInstall(test.sequence)
			assert.Equal(t, test.expected, result)
		})
	}
}
