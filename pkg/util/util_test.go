...
func Test_ReplicatedAPIEndpoint(t *testing.T) {
	tests := []struct {
		...
		{
			name: "license with full endpoint",
			license: &kotsv1beta1.License{
		...
		{
			name: "license with endpoint missing scheme",
			license: &kotsv1beta1.License{
				Spec: kotsv1beta1.LicenseSpec{
...
func Test_ProxyRegistryDomain(t *testing.T) {
	tests := []struct {
		name           string
		license        *kotsv1beta1.License
		isEmbedded     bool
	...
	}{
		{
			...
			isEmbedded:     false,
			expectedResult: "proxy.staging.replicated.com",
		},
		{
			name: "production license endpoint",
			license: &kotsv1beta1.License{
				Spec: kotsv1beta1.LicenseSpec{
					Endpoint: "https://replicated.app",
				},
		...
		{
			name:           "no license uses default domain",
			license:        nil,
			isEmbedded:     false,
			expectedResult: "proxy.replicated.com",
		},
		{
			name:           "embedded cluster with env domain",
			license:        nil,
			isEmbedded:     true,
			envDomain:      "proxy.custom.com",
			expectedResult: "proxy.custom.com",
		},
		{
			name:        "embedded cluster without env domain",
			license:     nil,
			isEmbedded:  true,
			expectError: true,
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.isEmbedded {
				...
				if test.envDomain != "" {
					t.Setenv("PROXY_REGISTRY_DOMAIN", test.envDomain)
				}
			}

			result, err := ProxyRegistryDomain(test.license)
...
func Test_ReplicatedRegistryDomain(t *testing.T) {
	tests := []struct {
		name           string
		license        *kotsv1beta1.License
		expectedResult string
	...
	}{
		{
			...
			expectedResult: "registry.staging.replicated.com",
		},
		{
			name: "production license endpoint",
			license: &kotsv1beta1.License{
			...
			expectedResult: "registry.replicated.com",
		},
		{
			name:           "no license uses default domain",
			license:        nil,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			result, err := ReplicatedRegistryDomain(test.license)
