...
func deployVersionForApp(clusterSocket *ClusterSocket, a *apptypes.App, deployedVersion *downstreamtypes.DownstreamVersion) error {
	...
	// JALAJA -
	// One for each hook we have to do kustomize build - Refer Ethan's PR to borrow code.
	// two routes - one is helm install and another is Ethan's where get the hooks logic.
	// need an opt-in for this method.
	// Always split the hooks into a different folder, otherwise all the files go into the same main folder
	// include all the bases in the kustomizatio helm-weight by default we should support with or without opt-in
	//
	...
	// JALAJA, this is the event sent to operator to deploy all the manifests generated by kotsadm
