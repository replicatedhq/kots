...
**Proposal.** Make KOTS version-aware so it emits the correct Velero install flags and `BackupStorageLocation` configuration for Velero 1.10 through 1.18. Restic support is preserved; customers can upgrade Velero at their own pace and existing Restic backups remain restorable. Customers using the Local Volume Provider (LVP) for HostPath/NFS/PVC backups must migrate to a Kopia-compatible destination before upgrading to Velero 1.17+, because LVP is not compatible with Kopia and existing LVP backups cannot be restored after the upgrade.
...
Minio is the default path for HostPath/NFS backups. The LVP plugin is only used when Minio has been explicitly disabled. Because of this, LVP-backed HostPath/NFS customers are a minority, but they are also the customers least likely to accept migrating to Minio (for example, due to AGPL licensing concerns).
...
5. Update Velero and plugin images to the target 1.17.x/1.18.x version.
6. Confirm with `velero version`.

KOTS provides install instructions in the Admin Console for new Velero installations. It does not provide upgrade instructions in the Admin Console. The Velero upgrade procedure is documented on the Replicated docs site and is performed by the cluster administrator.
